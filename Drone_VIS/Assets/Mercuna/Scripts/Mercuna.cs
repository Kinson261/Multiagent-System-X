...
namespace Mercuna
{
    ...
    public class Mercuna : MerSingleton<Mercuna>
    {
        ...
        [UnmanagedFunctionPointer(CallingConvention.StdCall)]
        delegate void fpLogToFile(int level, string msg);
        ...
        {
            Rigidbody rb = m_refTable.GetRigidbody(pEntity);
            if (rb)
            {
                vel = rb.velocity;
            }
        ...
        void EntityGetBounds(IntPtr pEntity, out MerVector center, out float radius)
        {
            ...
            Vector3 uCenter;
            GetCenterAndRadius(go, collider, out uCenter, out radius);
            center = uCenter;
        }
        ...
        {
            GameObject go = (GameObject)m_refTable.Get(pEntity);
            Mercuna3DNavigation navigationComponent = go.GetComponent<Mercuna3DNavigation>();
        ...
        internal static void GetCenterAndRadius(GameObject go, Collider collider, out Vector3 center, out float radius)
        {
            if (collider)
            {
                Vector3 scale = go.transform.localScale;  // TODO: Is this correct?!
                if (collider is SphereCollider)
                {
                    SphereCollider sphereCollider = (SphereCollider)collider;
                    Bounds bounds = collider.bounds;
                    center = bounds.center;
                    radius = sphereCollider.radius * Mathf.Max(scale.x, scale.y, scale.z);
                }
                else
                {
                    Bounds bounds = collider.bounds;
        ...
        {
            ThreadPool.QueueUserWorkItem(DoJobInternal, pJob);
        }
        ...
        [MonoPInvokeCallback]
        private static void SLogToFile(int level, string msg)
        {
            mercunaSingleton.m_logger.LogToFile(level, msg);
        }
        [MonoPInvokeCallback]
