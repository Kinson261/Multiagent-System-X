...
namespace Mercuna
{
    ...
    public class MercunaNavOctree : MonoBehaviour
    {
        ...
        [Tooltip("The minimum agent radius determine what navigation data is stored in the octree. Paths will never go closer to geometry than the minimum agent radius"), Range(1, 6)]
        public int minAgentRadius = 1;

        [Tooltip("The maximum agent radius determine what navigation data is stored in the octree. The octree doesn't store data to allow paths to be found with more clearance from geometry than the maximum radius."), Range(1, 29)]
        public int maxAgentRadius = 2;

        public static MercunaNavOctree GetInstance()
        {
            return FindObjectOfType<MercunaNavOctree>();
        }
        ...
        public void Build()
        {
            ...
            NavMeshBuilder.CollectSources(totalBounds, ~0, NavMeshCollectGeometry.PhysicsColliders, 0, markups, sources);

            GeometrySource[] sourceArray = new GeometrySource[sources.Count];
            int numSources = 0;
            for (int i = 0, iEnd = sources.Count; i < iEnd; i++)
            {
                if (sources[i].component.gameObject.GetComponent<Mercuna3DNavigation>() == null && 
                    sources[i].component.gameObject.GetComponent<MercunaObstacle>() == null)
                {
                    IntPtr h = Mercuna.instance.GCCreateRef(sources[i]);
                    Matrix4x4 transform = sources[i].transform;
                    ...
                    else if (sources[i].shape == NavMeshBuildSourceShape.Mesh)
                    {
                        ...
                        position += transform.MultiplyVector(((Mesh)sources[i].sourceObject).bounds.center);
                    }
